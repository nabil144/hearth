...
- `app/Sources/Store.swift`. Load and save the document. `@Observable`, or it will not travel through `.environment`. On decode failure, move the file aside (`progress.broken-<unix>.json`) instead of overwriting it. Same pattern as Life is a Game `world.json`.

## Data structures

`Progress` is the document. `CardRecord` holds `due`, `held`, `missed`. Session quiz cursor stays in memory, same as `app/app.js` `session`. It is not persisted.
